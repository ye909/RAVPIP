async function a() {
  ...
  <div>
      ...
      item.modelo
                                 ...
                                 <option value="${item.frecuencia_turbo}">${
      item.frecuencia_turbo
    }</option>
                                  <option value="${item.pulgadas}">${
      item.pulgadas
    }</option>
                                   <option value="${item.tasa_refresco}">${
      item.tasa_refresco
    }</option>
                                    <option value="${item.resolucion}">${
      item.resolucion
    }</option>
                                     <option value="${item.tipo_pantalla}">${
      item.tipo_pantalla
    }</option>
                                     <option value="${item.graficos}">${
      item.graficos
    }</option>
                                    <option value="${item.procesador}">${
      item.procesador
    }</option>
                                     <option value="${item.almacenamiento}">${
      item.almacenamiento
    }</option>
                                    <option value="${item.memoria_ram}">${
      item.memoria_ram
    }</option>
                                        <option value="${item.capacidad}">${
      item.capacidad
    }</option>
                                         <option value="${item.tipo_memoria}">${
      item.tipo_memoria
    }</option>
                                <option value="${item.nucleos} ">${
      item.nucleos
    } </option>
                                <option value="${item.hilos}">${
      item.hilos
    }</option>
                                    <option value="${item.frecuencia}">${
      item.frecuencia
    }</option>
                                     <option value="${item.cantidad}">${
      item.cantidad
    }</option>
                                   <option value="${
                                     item.tipo_almacenamiento
                                   }">${item.tipo_almacenamiento}</option>
                                                    <option value="${
      ...
      item.velocidad_escritura
                                ...
                                <option value="${item.frecuencia_base}">${
      item.frecuencia_base
    }</option>
                            </select>
