...
let productos = []; // guardamos los datos globalmente

// Cargamos los productos una vez
async function a() {
    const res = await fetch("/src/js/produtosAlquiler.json");
    productos = await res.json(); // guardamos todo el json para filtrarlo luego
}

a(); // ejecutamos la carga

// Filtra los productos por categoría y los muestra
function filtrarPorCategoria(categoria) {
    const rentalform = document.getElementById("rental-form");
    rentalform.innerHTML = ""; // Limpiar el contenido anterior

    const filtrados = productos.filter(item => item.tipo === categoria);
    ...
    filtrados.forEach((item, index) => {
        ...
        formcontainer.innerHTML = `
            <label>ID del Producto: <p>${index + 1}</p></label>
            <label for="tipo">Tipo:</label>
            <select id="tipo">
                <option value="">Seleccione el producto</option>
                <option value="${item.nombre}">${item.nombre}</option>
                ${item.marca ? `<option value="${item.marca}">${item.marca}</option>` : ''}
                ${item.modelo ? `<option value="${item.modelo}">${item.modelo}</option>` : ''}
                ${item.precio ? `<option value="${item.precio}">${item.precio}</option>` : ''}
                ${item.capacidad ? `<option value="${item.capacidad}">${item.capacidad}</option>` : ''}
                ${item.frecuencia ? `<option value="${item.frecuencia}">${item.frecuencia}</option>` : ''}
                <!-- Agrega aquí más campos si deseas -->
            </select>
        `;
