

fetch("../js/datos.json")
  .then(response => response.json())
  .then(data =>{

  data.forEach((item)=>{
    const productlist = document.getElementById("product-list")
    const sudcontainer=document.createElement("div")
    sudcontainer.setAttribute("data-id", item.id); // Suponiendo que el JSON tiene un campo "id"
    sudcontainer.setAttribute("data-name", item.name);
    sudcontainer.setAttribute("data-price", item.price);

    
    sudcontainer.classList.add("product")
    sudcontainer.innerHTML=`
    <img src="${item.image}" alt="Smartphone X">
   <h3>${item.name} </h3>
   <p>$ ${item.price}</p>

    `
    const boton= document.createElement("button")
    boton.classList.add("add-to-cart")
    boton.innerText="Agregar al Carrito"

    sudcontainer.appendChild(boton)

   productlist.appendChild(sudcontainer)
    console.log(productlist)
   
  })  
 

  


  const cart = [];
  let cartTotal = 0;

// Función para actualizar el carrito
 function updateCart() {
   const cartItemsDiv = document.getElementById('cart-items');
   const cartCountSpan = document.getElementById('cart-count');
   const cartTotalSpan = document.getElementById('cart-total');
   const checkoutBtn = document.getElementById('checkout-btn');

  // Limpiar el contenido actual del carrito
   cartItemsDiv.innerHTML = '';

   if (cart.length === 0) {
     cartItemsDiv.innerHTML = '<p>No hay productos en el carrito.</p>';
     checkoutBtn.disabled = true;
   } else {
     cart.forEach(item => {
       const itemDiv = document.createElement('div');
       itemDiv.textContent = `${item.name} - $${item.price}`;
       cartItemsDiv.appendChild(itemDiv);
     });
     checkoutBtn.disabled = false;
   }

   // Actualizar el contador y el total
   cartCountSpan.textContent = cart.length;
   cartTotalSpan.textContent = cartTotal.toFixed(2);
 }

 // Agregar productos al carrito
 document.querySelectorAll('.add-to-cart').forEach(button => {
   button.addEventListener('click', () => {
     const productDiv = button.parentElement;
     const productId = productDiv.dataset.id;
     const productName = productDiv.dataset.name;
     const productPrice = parseFloat(productDiv.dataset.price);

//     // Verificar si el producto ya está en el carrito
     const existingItem = cart.find(item => item.id === productId);
     if (existingItem) {
       existingItem.quantity += 1;
     } else {
       cart.push({ id: productId, name: productName, price: productPrice, quantity: 1 });
     }

     // Actualizar el total
     cartTotal += productPrice;

     // Actualizar la vista del carrito
     updateCart();
   });
 });

 document.getElementById("checkout-btn").addEventListener("click",verCarrito)

 function verCarrito() {
  alert("a")
  // const containerCarrito

 }

// document.getElementById("boton").addEventListener("click",)
 document.getElementById("close").addEventListener("click",close)
 document.getElementById("imgcarro").addEventListener("click",verCarrito)

 function verCarrito() {
  const carrito=document.getElementById("carrito")
  carrito.style.display="inline"
  document.getElementById("boton").value
 }
function close() {
  const carrito=document.getElementById("carrito")

 carrito.style.display="none"
}
} )
 // Inicializar el carrito
 updateCart();


// Al final de tu archivo ventas.js, añade este código:

// Variable para almacenar el total de ventas
let totalVentas = 0;

// Obtener el botón de comprar ahora
const checkoutBtn = document.getElementById('checkout-btn');

// Función para actualizar el contador de ventas en todos los archivos
function actualizarEstadoVentas() {
    // Actualizar en el localStorage
    localStorage.setItem('totalVentas', totalVentas);
    
    // Buscar el elemento estadoVentas en otros archivos
    const estadoVentasElements = document.querySelectorAll('#estadoVentas p');
    estadoVentasElements.forEach(element => {
        element.textContent = totalVentas;
    });
}

// Evento al hacer clic en Comprar Ahora
checkoutBtn.addEventListener('click', function() {
    // Obtener la cantidad actual del carrito
    const cartCount = parseInt(document.getElementById('cart-count').textContent);
    
    // Solo incrementar si hay productos en el carrito
    if (cartCount > 0) {
        totalVentas += cartCount;
        
        // Actualizar el contador de ventas
        actualizarEstadoVentas();
        
        // Mostrar mensaje de confirmación
        alert(`¡Compra realizada! Se han agregado ${cartCount} productos a las ventas totales.`);
        
        // Vaciar el carrito
        vaciarCarrito();
    } else {
        alert('No hay productos en el carrito.');
    }
});

// Función para vaciar el carrito
function vaciarCarrito() {
    // Tu lógica actual para vaciar el carrito
    document.getElementById('cart-items').innerHTML = '<p>No hay productos en el carrito.</p>';
    document.getElementById('cart-count').textContent = '0';
    document.getElementById('cart-total').textContent = '0';
    
    // Aquí deberías también limpiar cualquier array o objeto que uses para almacenar los items
}

// Al cargar la página, recuperar el total de ventas del localStorage
document.addEventListener('DOMContentLoaded', function() {
    const ventasGuardadas = localStorage.getItem('totalVentas');
    if (ventasGuardadas) {
        totalVentas = parseInt(ventasGuardadas);
        actualizarEstadoVentas();
    }
});